if IY_LOADED and not _G.IY_DEBUG == true then
    -- error("Pluto Yield is already running!", 0)
    return
end
...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "Pluto Yield is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "Pluto Yield FE v" .. currentVersion
...
Logo.Image = "rbxassetid://1352543373"
...
PluginsHint.Text = "Download plugins from the IY Discord (discord.gg/getpluto)"
...
reference = (function()
	local main = create({
		...
		{114,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Text",Parent={112},Position=UDim2.new(0,8,0,30),Size=UDim2.new(1,-8,0,32),Text="You can join the Discord server to get support with PY,  and read up on more documentation such as the Plugin API.",TextColor3=Color3.new(1,1,1),TextSize=14,TextWrapped=true,TextXAlignment=0,ZIndex=10,}},
	...
	inviteButton.MouseButton1Click:Connect(function()
		if everyClipboard then
			toClipboard("https://discord.gg/getpluto")
		...
		inviteButton.Text = "Copy Discord Invite Link (https://discord.gg/getpluto)"
	end)
	dragGUI(main)
	main.Parent = PARENT
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Pluto Yield Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the Pluto Yield support server.'}
...
CMDs[#CMDs + 1] = {NAME = 'fuck [player] [speed]', DESC = 'owo'}
CMDs[#CMDs + 1] = {NAME = 'unfuck', DESC = 'uwu'}
...
addcmd('discord', {'support', 'help'}, function(args, speaker)
	if everyClipboard then
		toClipboard('https://discord.com/invite/getpluto')
		notify('Discord Invite', 'Copied to clipboard!\ndiscord.gg/getpluto')
	else
		notify('Discord Invite', 'discord.gg/getpluto')
	end
	if httprequest then
		httprequest({
...
addcmd("Fuck", {"rape"}, function(args, speaker)
	execCmd("unfuck")
	wait()
	local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
...
task.spawn(function()
	...
	if success then
		if currentVersion ~= latestVersionInfo.Version then
			notify('This was not cracked I was just bored','Discord.gg/getpluto')
		end
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, Pluto Yield may have issues or features that are not functioning correctly.") end
end)
